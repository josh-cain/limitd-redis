...
class LimitDBRedis extends EventEmitter {
  ...
  buildStatusResponse(type, bucket, prefixLength) {
    return (key, cb) => {
      key = key.substring(prefixLength);
      const bucketKeyConfig = this.bucketKeyConfig(bucket, key);
      this.redis.hmget(`${type}:${key}`, 'd', 'r', (err, hash) => {
        if (err) {
          return cb(err);
        }
        const lastDrip = parseInt(hash[0], 10);
        const remaining = parseInt(hash[1], 10);
        return cb(null, {
          key,
          instance: key,
          limit: bucketKeyConfig.size,
  ...
  status(params, callback) {
    ...
    const transform = this.buildStatusResponse(params.type, bucket, prefixLength);

    const matches = [];
    let errors = [];
    const stream = this.scanStream({ prefix });
    ...
    stream.on('end', () => {
      async.map(matches, transform, (err, items) => {
        return callback(err, { items, errors });
      });
    });
  }

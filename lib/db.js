...
class LimitDBRedis extends EventEmitter {
  ...
  buildStatusResponse(bucket, { key, remaining }) {
  ...
  scanStream(params) {
    ...
    return mergeStream(nodes.map((node) => {
      return node.scanStream({
        match: params.prefix,
        count: 100
      });
    }));
  ...
  status(params, callback) {
    ...
    const rStream = this.scanStream({ prefix });
    const flattenStream = new streamUtils.BreakReadBatchStream({ limit: count });
    const keyInfoStream = new streamUtils.StatusKeyDetails({ redis: this.redis, type, prefixLength});
    ...
    flattenStream.on('error', (err) => {
      errors.push(err);
    });
    keyInfoStream.on('error', (err) => {
      errors.push(err);
    })

    rStream.pipe(flattenStream).pipe(keyInfoStream)
      .on('data', (data) => {
        if (items.length < count) {
          items.push(this.buildStatusResponse(bucket, data));
        }
      }).on('end', () => {
